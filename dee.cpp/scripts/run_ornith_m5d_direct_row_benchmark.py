#!/usr/bin/env python3
"""Paired M5D native-combined vs direct-single-row full-model benchmark."""
...
CONTROL = "native-combined"
CANDIDATE = "native-combined-direct"
...
def main() -> int:
    ...
    if gpu_count != 2:
        raise RuntimeError(f"M5D benchmark requires exactly two GPUs, got {gpu_count}")
    ...
    expected_rows = layer_count * len(EXPECTED_TOKENS) * topk
    expected_gather_bytes = expected_rows * hidden * np.dtype(np.float16).itemsize
    expected_scatter_bytes = expected_rows * hidden * np.dtype(np.float32).itemsize
    ...
    control_trace_row, control_trace = run_m5d_once(
        runtime,
        tokenizer,
        CONTROL,
        trace=True,
    ...
    candidate_trace_row, candidate_trace = run_m5d_once(
        runtime,
        tokenizer,
        CANDIDATE,
        trace=True,
    ...
    gates = {
        ...
        "control_uses_exact_combined_path": all(
            row["path_proof"]["native_combined_calls"] == 160
            and row["path_proof"]["native_direct_calls"] == 0
            for row in control_rows
        ),
        "candidate_uses_direct_path_for_all_layers": all(
            row["path_proof"]["native_combined_calls"] == 160
            and row["path_proof"]["native_direct_calls"] == 160
            and row["path_proof"]["native_direct_fallback_calls"] == 0
            for row in candidate_rows
        ),
        "control_d2d_copy_contract_exact": all(
            row["engine_io_delta"]["d2d_gather_copies"] == expected_rows
            and row["engine_io_delta"]["d2d_gather_bytes"]
            == expected_gather_bytes
            and row["engine_io_delta"]["d2d_scatter_copies"] == expected_rows
            and row["engine_io_delta"]["d2d_scatter_bytes"]
            == expected_scatter_bytes
            for row in control_rows
        ),
        "candidate_eliminates_all_row_d2d_copies": all(
            row["engine_io_delta"]["d2d_gather_copies"] == 0
            and row["engine_io_delta"]["d2d_gather_bytes"] == 0
            and row["engine_io_delta"]["d2d_scatter_copies"] == 0
            and row["engine_io_delta"]["d2d_scatter_bytes"] == 0
            and row["engine_io_delta"]["direct_row_gather_bypasses"]
            == expected_rows
            and row["engine_io_delta"]["direct_row_scatter_bypasses"]
            == expected_rows
            for row in candidate_rows
        ),
    ...
    report = {
        ...
        "copy_contract": {
            "expected_rows_per_generation": expected_rows,
            "expected_control_gather_bytes": expected_gather_bytes,
            "expected_control_scatter_bytes": expected_scatter_bytes,
        },
        "pareto": {
            "control_native_combined_median_tps": control_tps,
            "candidate_direct_row_median_tps": candidate_tps,
        ...
        "implementation_boundary": {
            "control": (
                "One cudaMemcpyAsync gather and scatter per selected expert "
                "row around otherwise identical cuBLAS expert math."
            ),
            "candidate": (
                "Singleton expert groups consume the source hidden row and "
                "write the raw routed slot directly; duplicate/multi-row "
                "groups retain the established copy path."
            ),
        },
    }
    report_path = args.output_dir / "m5d-direct-row-benchmark.json"

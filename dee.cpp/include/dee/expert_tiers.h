...
struct Phase2TierConfig {
    bool enabled = false; // master switch; subfeatures remain independently selectable
    bool host_enabled = false; // explicit host tier arm
    bool vram_priority_fix_enabled = false; // explicit actual-recency VRAM arm
    std::string model_identity; // immutable checkpoint ID/revision supplied by caller
    HostTierConfig host;
    std::shared_ptr<const HostPlacementPolicy> host_policy; // null => plain LRU

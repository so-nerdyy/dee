...
class DeepseekV4Compressor:
    ...
    def forward(self, x: torch.Tensor, start_pos: int,
        ...
        dtype = x.dtype
        xf = x.float()
        kv = xf @ w["wkv.weight"].transpose(0, 1)
...
class DeepseekV4Indexer:
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, offset: int,
        ...
        q = (qr.float() @ w["wq_b.weight"].transpose(0, 1)).to(qr.dtype)
...
class DeepseekV4Attention:
    def __init__(self, cfg: LayerConfig, *, device: str = "cpu", max_batch: int = 1):
        ...
        self.device = device
        self.max_batch = max_batch
        self.compressor = DeepseekV4Compressor(cfg, cfg.head_dim, device=device,
                                               max_batch=max_batch)
        self.indexer = DeepseekV4Indexer(cfg, device=device, max_batch=max_batch)
    ...
    def forward(self, x: torch.Tensor, start_pos: int, w: dict[str, torch.Tensor],
        ...
        qr = common.rms_norm(
            (x.float() @ w["wq_a.weight"].transpose(0, 1)).to(x.dtype),
            w["q_norm.weight"], cfg.norm_eps)
        q = (qr.float() @ w["wq_b.weight"].transpose(0, 1)).to(x.dtype)
        ...
        common.apply_rotary_emb(q[..., -rd:], freqs_cis)
        # kv path
        kv = (x.float() @ w["wkv.weight"].transpose(0, 1)).to(x.dtype)
        ...
        if start_pos == 0:
            ...
            if capture is not None:
                capture["kv_win"] = kv
            kv_compress = self.compressor.forward(x, start_pos, w["compressor"],
                                                  capture)
        ...
        o = o.to(x.dtype)
        out = (o.flatten(2).float() @ w["wo_b.weight"].transpose(0, 1)).to(x.dtype)
        if capture is not None:
            capture["qr"] = qr
            capture["q"] = q
            capture["kv"] = kv
...
class DeepseekV4Layer:
    ...
    def _hc_pre(self, x: torch.Tensor, hc_fn: torch.Tensor, hc_scale: torch.Tensor,
        ...
        mixes = xf @ hc_fn.transpose(0, 1) * rsqrt
    ...
    def state_signature(self) -> dict[str, str]:
        attn = self.attn
        sig = {
            "attn_kv_cache": tensor_sha256(attn.kv_cache),
            "compressor_kv_state": tensor_sha256(attn.compressor.kv_state),
            "compressor_score_state": tensor_sha256(attn.compressor.score_state),
            "indexer_kv_cache": tensor_sha256(attn.indexer.kv_cache),
            "indexer_compressor_kv_state": tensor_sha256(
                attn.indexer.compressor.kv_state),
            "indexer_compressor_score_state": tensor_sha256(
...
def build_layer_weights_from_tensors(raw: dict[str, torch.Tensor], layer: int = 20) -> dict[str, torch.Tensor]:
    ...
    w["ffn"] = {
        ...
        "gate_b": dense(f"{p}.ffn.gate.bias"),
...
def make_synthetic_layer_weights(cfg: LayerConfig, *, seed: int = 0,
    ...
    attn = {
        ...
        "compressor": {
            "wkv.weight": rand(2 * hd, d),
            "wgate.weight": rand(2 * hd, d),
            "norm.weight": rand(hd),
            "ape": rand(cfg.compress_ratio, 2 * hd),
        },
        "indexer": {

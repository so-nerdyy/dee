#!/usr/bin/env python3
"""Authoritative dual-T4 M5F pointer-batched expert experiment."""
...
EVIDENCE = Path(f"/kaggle/working/ornith-m5f-evidence-{RUN_ID}")
ARCHIVE_BASE = Path("/kaggle/working/ornith-m5f-evidence")
...
try:
    ...
    environment = {
        ...
        "candidate_claim_scope": (
            "Token-1 all-unique selected experts use three device-pointer "
            "cublasGemmBatchedEx projection calls plus one activation launch. "
            "Router IDs, cache policy, FP16 boundaries, FP32 raw outputs, "
            "weighted combine, and stream handoff remain exact."
        ),
    }
    ...
    benchmark_rc = run_tee(
        [
            ...
            "-X",
            "faulthandler",
            str(dee / "scripts/run_ornith_m5f_pointer_batched_benchmark.py"),
            "--model-dir",
            str(model),
            "--output-dir",
            str(benchmark_dir),
        ...
        EVIDENCE / "logs/m5f.log",
        dee,
        check=False,
    )
    benchmark_required = [
        benchmark_dir / "m5f-pointer-batched-benchmark.json",
    ...
    if all(path.is_file() for path in benchmark_required):
        report = json.loads(
            (benchmark_dir / "m5f-pointer-batched-benchmark.json").read_text()
        )
        gates = report.get("pareto", {}).get("gates", {})
        candidate_accepted = bool(gates) and all(gates.values())
        rejected_gates = sorted(
            name for name, value in gates.items() if value is not True
        )
        candidate_summary = {
            "candidate_accepted": candidate_accepted,
            "report_result": report.get("result"),
            "control_median_tps": report.get("pareto", {}).get(
                "control_direct_median_tps"
            ),
            "candidate_median_tps": report.get("pareto", {}).get(
                "candidate_pointer_batched_median_tps"
            ),
            "speedup_percent": report.get("pareto", {}).get(
                "speedup_percent"
            ),
            "rejected_gates": rejected_gates,
            "launch_contract": report.get("launch_contract"),
            "pointer_workspace_contract": report.get(
                "pointer_workspace_contract"
            ),
        ...
        require(
            "m5f_report_has_gates",
            bool(gates),
            gates,
        )
        require(
            "m5f_result_matches_acceptance",
            (report.get("result") == "PASS") == candidate_accepted,
            candidate_summary,
        )
        require(
            "m5f_return_code_matches_acceptance",
        ...
        require(
            "m5f_required_artifacts_present",
            False,
            [
                str(path)

#!/usr/bin/env python3
"""Authoritative dual-T4 M5C exact native-combined A/B."""
...
ARCHIVE_BASE = Path("/kaggle/working/ornith-m5c-evidence")
...
try:
    ...
    environment = {
        ...
        "gpus": bootstrap["gpus"],
        "hardware_counter_tools": {
            name: tool_inventory(name) for name in ("ncu", "nsys")
        },
    ...
    benchmark_rc = run_tee(
        [
            ...
            "-X",
            "faulthandler",
            str(dee / "scripts/run_ornith_m5c_combined_benchmark.py"),
            "--model-dir",
            str(model),
            "--output-dir",
            str(benchmark_dir),
        ...
        EVIDENCE / "logs/m5c.log",
        dee,
        check=False,
    )
    benchmark_required = [
        benchmark_dir / "m5c-native-combined-benchmark.json",
    ...
    if all(path.is_file() for path in benchmark_required):
        report = json.loads(
            (benchmark_dir / "m5c-native-combined-benchmark.json").read_text()
        )
        ...
        candidate_summary = {
            ...
            "control_median_tps": report.get("pareto", {}).get(
                "control_raw_python_median_tps"
            ),
            "candidate_median_tps": report.get("pareto", {}).get(
                "candidate_native_combined_median_tps"
            ),
        ...
        require(
            "m5c_required_artifacts_present",
            False,
            [
                str(path)

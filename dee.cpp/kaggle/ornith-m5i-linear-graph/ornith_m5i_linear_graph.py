#!/usr/bin/env python3
"""Authoritative dual-T4 M5I linear-attention CUDA Graph experiment."""
...
RUN_ID = "20260730T213000Z-m5i-v1"
EXPECTED_COMMIT = "9d04317777ccf8c6f0c5f914cf2fc6246a5e6214"
BRANCH = "codex/phase2-cap32-matrix"
ROOT = Path("/kaggle/temp/dee-source")
EVIDENCE = Path(f"/kaggle/working/ornith-m5i-evidence-{RUN_ID}")
ARCHIVE_BASE = Path("/kaggle/working/ornith-m5i-evidence")
...
try:
    ...
    if len(shards) != 16 or not all((model / name).is_file() for name in shards):
    ...
    environment = {
        ...
        "candidate_claim_scope": (
            "Capture only the 30 batch-one cached-decode linear-attention "
            "modules with static CUDA Graph addresses. Keep eager prefill, "
            "top-8 router, accepted pointer-batched experts, model, tokens, "
            "precision, split, and cache cap fixed."
        ),
    }
    ...
    benchmark_rc = run_logged(
        [
            ...
            "-X",
            "faulthandler",
            str(dee / "scripts/run_ornith_m5i_linear_graph_benchmark.py"),
            "--model-dir",
            str(model),
            "--output-dir",
            str(benchmark_dir),
        ...
        EVIDENCE / "logs/m5i.log",
        dee,
        check=False,
    )
    benchmark_required = [
        benchmark_dir / "m5i-linear-graph-benchmark.json",
        benchmark_dir / "artifact-manifest.json",
    ]
    required_paths.extend([EVIDENCE / "logs/m5i.log", *benchmark_required])
    if all(path.is_file() for path in benchmark_required):
        report = json.loads(
            (benchmark_dir / "m5i-linear-graph-benchmark.json").read_text()
        )
        ...
        require(
            "m5i_result_matches_acceptance",
            (report.get("result") == "PASS") == candidate_accepted,
            candidate_summary,
        )
        require(
            "m5i_return_code_matches_acceptance",
        ...
        require(
            ...
            and report.get("runtime_identity", {}).get("git_commit") == EXPECTED_COMMIT,
            report.get("runtime_identity"),
        )
        require(
            "m5i_graph_scope_exact",
            report.get("configuration", {}).get("linear_attention_layer_count") == 30
            and report.get("configuration", {}).get("expert_execution_mode")
            == "native-combined-pointer"
            and report.get("configuration", {}).get("topk") == 8,
            report.get("configuration"),
        )
    else:
        require(
            "m5i_required_artifacts_present",
            False,
            [str(path) for path in benchmark_required if not path.is_file()],
        )
    ...
    missing_required = [row["path"] for row in required_status if not row["present"]]
    if missing_required:
    ...
    final_result = "PASS" if fatal_error is None and not validation_failures else "FAIL"
    artifacts = [
        {

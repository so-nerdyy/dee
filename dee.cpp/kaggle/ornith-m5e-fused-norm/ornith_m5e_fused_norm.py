#!/usr/bin/env python3
"""Authoritative dual-T4 M5E scoped fused-normalization experiment."""
...
RUN_ID = "20260729T214842Z-m5e-v2"
EXPECTED_COMMIT = "e2aaeb42362814d75d6bfe86ae5801a22956512a"
BRANCH = "codex/phase2-cap32-matrix"
ROOT = Path("/kaggle/temp/dee-source")
EVIDENCE = Path(f"/kaggle/working/ornith-m5e-evidence-{RUN_ID}")
ARCHIVE_BASE = Path("/kaggle/working/ornith-m5e-evidence")
...
try:
    ...
    environment = {
        ...
        "candidate_claim_scope": (
            "Partial layer/final/gated norm fusion; full-attention q/k head "
            "norms remain eager and are explicitly inventoried."
        ),
    }
    ...
    benchmark_rc = run_tee(
        [
            ...
            "-X",
            "faulthandler",
            str(dee / "scripts/run_ornith_m5e_fused_norm_benchmark.py"),
            "--model-dir",
            str(model),
            "--output-dir",
            str(benchmark_dir),
        ...
        EVIDENCE / "logs/m5e.log",
        dee,
        check=False,
    )
    benchmark_required = [
        benchmark_dir / "m5e-fused-norm-benchmark.json",
    ...
    if all(path.is_file() for path in benchmark_required):
        report = json.loads(
            (benchmark_dir / "m5e-fused-norm-benchmark.json").read_text()
        )
        ...
        candidate_summary = {
            ...
            "control_median_tps": report.get("pareto", {}).get(
                "control_eager_norm_median_tps"
            ),
            "candidate_median_tps": report.get("pareto", {}).get(
                "candidate_fused_norm_median_tps"
            ),
            "speedup_percent": report.get("pareto", {}).get(
                "speedup_percent"
            ),
            "rejected_gates": rejected_gates,
            "native_contract_probe": report.get(
                "normalization_contract", {}
            ).get("native_contract_probe"),
            "normalization_inventory": report.get(
                "normalization_contract", {}
            ).get("inventory"),
        ...
        require(
            "m5e_report_has_gates",
            bool(gates),
            gates,
        )
        require(
            "m5e_result_matches_acceptance",
            (report.get("result") == "PASS") == candidate_accepted,
            candidate_summary,
        )
        require(
            "m5e_return_code_matches_acceptance",
        ...
        require(
            "m5e_required_artifacts_present",
            False,
            [
                str(path)

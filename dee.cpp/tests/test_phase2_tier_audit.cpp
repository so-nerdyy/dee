...
//
// Adversarial audit tests for the Phase-2 host/device tier seams, ported from
// research/phase2-host-tier-verification @ e63a3bc (PHASE2_VERIFICATION_AUDIT.md)
// onto Luna's rewrite (research/phase2-integration-lru-fix) with the audit's
// hardening findings applied. CPU-only: exercises HostExpertTier,
// HostExpertLease, IdentityCodec, ExpertStoreColdAdapter seams (via a fake
// ExpertStore), the AsyncPrefetcher managed-source path on the mock stream,
// VramCacheManager::discard_unpinned guards, and the new lifetime scope
// exclusivity (one-shot arming token + armed-scope epoch).
//
...
// full-width key identity on LLP64, second-live-tier rejection, and teardown
// with in-flight fills.
...
// ===========================================================================
// 1. map_key is now unconditionally the full 64-bit key_id (audit M1 fix).
//    Pre-hardening, the legacy OFF path computed (uint64_t)(long)key which
//    collapsed every layer sharing an expert id on LLP64 (32-bit long) — a
//    fail-closed defect the audit reproduced as b == a on MinGW. The fix
//    removes the conditional entirely: "default-OFF equivalence" only ever
//    existed on LP64 where the cast is the identity, and the map is internal
//    state with no wire format.
// ===========================================================================
void mapkey_full_width_unconditional() {
    std::cout << "-- map_key full width on every path --\n";
    VramCacheManager cache;
    check(cache.init(256, mock_device()), "mapkey: cache init");
    AsyncPrefetcher pf(cache);
    check(pf.init(false), "mapkey: mock init");
    const uint8_t src[64] = {7};
    // Legacy OFF path: no experimental tier armed.
    const long a = pf.prefetch(0, 5, src, 64, 0);
    const long b = pf.prefetch(1, 5, src, 64, 0);
    check(a >= 0, "mapkey: first prefetch issued");
    check(b >= 0 && b != a, "mapkey: same-expert different-layer issues a distinct transfer");
    check(pf.wait(0, 5), "mapkey: layer-0 expert completes");
    check(pf.wait(1, 5), "mapkey: layer-1 expert completes (no silent coalescing)");
    check(cache.is_resident(0, 5) && cache.is_resident(1, 5),
          "mapkey: both layer blocks resident");
    check(pf.accounting_valid(), "mapkey: request accounting consistent");
}
...
void store_guard_matrix() {
    ...
    // Contract: the same key+original size keeps serving; only evict()
    // withdraws it, after which the new size may fill.
    good.lease.reset(); still.lease.reset();
    check(host.evict(rec(20).key), "guard: resident copy withdrawable");
    auto resized = host.acquire(bad, store, codec);
    check(resized.lease && resized.status == HostAcquireStatus::Ready,
          "guard: post-evict same-key different-size fill succeeds (Invalid is not permanent)");
...
// ===========================================================================
// 9. Lifetime scope exclusivity (audit M2 fix). The ctor-time-only guard used
//    to allow: tier1 alive -> drain -> prefetcher.reset() + cache.clear() ->
//    tier2 ctor succeeds -> armed scope overwritten -> tier1's resident-hit
//    stage() could observe blocks staged under a foreign scope. Now the
//    arming token is held for the tier's whole lifetime, so a second live
//    tier fails at construction under EVERY interleaving, and the armed-scope
//    epoch gives stage()/wait() a stale-scope fail-closed tripwire.
// ===========================================================================
void device_tier_scope_exclusivity() {
    std::cout << "-- device tier lifetime scope exclusivity --\n";
    IdentityCodec codec; ByteStore store; MemoryCounts mc;
    HostExpertTier host(cfg(4), counted_backend(mc), std::make_shared<AuditPolicy>());
    VramCacheManager cache;
    check(cache.init(512, mock_device()), "scope: cache init");
    AsyncPrefetcher pf(cache);
    check(pf.init(false), "scope: prefetcher init");
    const uint64_t epoch0 = pf.scope_epoch();
    auto foreign = rec(0); foreign.key.model = "model-B"; // same rep, other model
    {
        DeviceExpertTier dev(cache, pf, rec(0).key); // scope "audit-model"
        check(pf.scope_epoch() == epoch0 + 1,
              "scope: successful arming bumps the armed-scope epoch");
        // Second live tier on the same prefetcher — the no-reset interleaving.
        bool threw = false;
        try { DeviceExpertTier dev2(cache, pf, foreign.key); }
        catch (const std::invalid_argument&) { threw = true; }
        check(threw, "scope: second live tier rejected on a clean prefetcher");
        threw = false;
        try { DeviceExpertTier dev2(cache, pf, rec(0).key); }
        catch (const std::invalid_argument&) { threw = true; }
        check(threw, "scope: second live tier rejected even with identical scope");
        // Baseline: the owner tier still stages and waits.
        check(dev.stage(rec(1), store, host, codec, 0), "scope: owner tier stages");
        check(dev.wait(rec(1).key), "scope: owner tier waits");
        // The audit's exact interleaving: drain + reset + evict-all while the
        // first tier is alive must NOT free the arming token.
        pf.reset();
        cache.clear();
        threw = false;
        try { DeviceExpertTier dev2(cache, pf, foreign.key); }
        catch (const std::invalid_argument&) { threw = true; }
        check(threw, "scope: reset()+cache.clear() does not release a live tier's arming");
        // reset() is a routine drain in this codebase (Engine calls it
        // between sequences via reset_runtime_cache, mid-preload, and at
        // generate() teardown) — it must NOT stale the legitimately armed
        // tier. The epoch moves only when a scope is armed.
        check(dev.stage(rec(2), store, host, codec, 0),
              "scope: owner tier survives routine prefetcher.reset()");
        check(dev.wait(rec(2).key), "scope: owner wait survives reset");
        check(pf.scope_epoch() == epoch0 + 1,
              "scope: epoch stable while one scope stays armed");
    }
    // ~DeviceExpertTier released the token; a different scope may now arm.
    // The block staged above is still resident and its transfer still sits in
    // the in-flight table, so both must be cleared before the ctor's
    // emptiness gates — the same drain the Engine performs on re-init.
    pf.reset();
    cache.clear();
    {
        DeviceExpertTier dev2(cache, pf, foreign.key); // scope "model-B"
        check(pf.scope_epoch() == epoch0 + 2, "scope: re-arm under a new scope bumps epoch");
        auto b1 = foreign; b1.key.expert = 1;
        check(dev2.stage(b1, store, host, codec, 0),
              "scope: replacement tier stages under model-B");
        check(dev2.wait(b1.key), "scope: replacement tier waits");
        // The previously armed scope's leases are now foreign.
        auto stale = host.acquire(rec(3), store, codec); // audit-model record
        check(stale.lease && pf.prefetch_host_lease(stale.lease, 0) < 0,
              "scope: stale-scope host lease rejected after re-arm");
    }
    // NOTE on the stale-scope stage()/wait() rejection itself: with the
    // one-shot token, no live tier can observe a bumped epoch — a re-arm can
    // only succeed after ~DeviceExpertTier releases the token, at which point
    // the stale tier no longer exists to call stage(). The epoch check in
    // stage()/wait() is the belt to the token's suspenders: it fires only if
    // a future arming path bypasses the token.
}

// ===========================================================================
// 10. Armed experimental state outlives DeviceExpertTier — DOCUMENTED
//     one-shot-per-prefetcher contract (audit L3): the armed flag and last
//     scope persist so prefetch_host_lease stays scoped, and (pre-hardening)
//     map_key stays 64-bit — now unconditional anyway. The synchronize_all
//     fail-stop is keyed on managed transfers actually in flight, not on the
//     flag, so a stale armed flag cannot upgrade a legacy drain failure into
//     an abort (that path is DEE_CUDA-only; asserted here only by contract).
// ===========================================================================
void experimental_flag_persistence() {
    ...
    const uint64_t epoch0 = pf.scope_epoch();
    { DeviceExpertTier dev(cache, pf, rec(0).key); }
    check(pf.scope_epoch() == epoch0 + 1, "persist: epoch stayed bumped after teardown");
    // After the tier died, a scope-matching lease is still accepted by
    // prefetch_host_lease (armed flag + scope are never cleared — documented
    ...
    // map_key is unconditionally 64-bit now: legacy calls never collide.
    const uint8_t src[64] = {0};
    const long x = pf.prefetch(0, 6, src, 64, 0);
    const long y = pf.prefetch(1, 6, src, 64, 0);
    check(x >= 0 && y >= 0 && x != y, "persist: full-width keys on legacy path too");
}

// ===========================================================================
// 11. Concurrency stress: N threads, same key -> exactly one fill; distinct
...
// ===========================================================================
// 12. Mock managed-path details: collect() is a no-op without wait_one;
...
int main() {
    mapkey_full_width_unconditional();

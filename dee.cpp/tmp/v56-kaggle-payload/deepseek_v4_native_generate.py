...
# v56 launch payload: keep the cloned source pinned to the v56 configuration
# commit while making the requested profiled configuration explicit. The shared
# checkout remains untouched; main() writes this exact config into the freshly
# cloned tree before apply_run_config(), so evidence records the actual v56
# values.
V56_RUN_CONFIG = {
    "_comment": "Run the 16-token sealed gate against the trace-guided DEE4 v3 store with bounded parallel host fills and full stage profiling.",
    "run_id": "p2.3-dee4-trace-fillpool-profile-v56",
    "cache_dtype": "fp4",
...
# Kaggle does not reliably propagate kernel env vars; pin this payload to the
# exact source commit requested for v56 while still allowing an explicit env
# override for controlled diagnostics.
COMMIT = os.environ.get(
    "NATIVE_COMMIT", "b29cf851321acc7402802ea69fc1eb60eab3a8d8")
...
def main() -> int:
    ...
    cloned_run_config.write_text(
        json.dumps(V56_RUN_CONFIG, indent=2) + "\n", encoding="utf-8")
    log(f"[config] injected v56 run_config at {cloned_run_config}")
    apply_run_config()

...
bool Engine::prepare_fp4_experts(
    ...
    if (cfg_.phase2.enabled) return true;
...
bool Engine::init(const EngineConfig& cfg) {
    ...
    if (cfg.phase2.enabled && (!fp4_cache || !cfg.use_cuda ||
        cfg.transfer_dtype != WeightTransferDType::Fp4E2m1 ||
        cfg.phase2.model_identity.empty() || !cfg.phase2.host_policy)) {
        std::fprintf(stderr, "[engine] Phase 2 requires packed FP4 CUDA, an immutable model identity, and an explicit host policy\n");
        return false;
    }
    ...
    if (cfg_.phase2.enabled) {
        ...
        try {
            ...
            phase2_host_ = std::make_unique<HostExpertTier>(host_config,
                host_memory_backend(cfg_.use_cuda, cfg_.device_id), cfg_.phase2.host_policy);

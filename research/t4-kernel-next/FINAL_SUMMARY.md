...
9. **Single best next microbench:** `kaggle_microbench_fp4_decode.py`
   (vectorized nibble loads, bitwise gate, 3 cases incl. real w1 bytes) —
   small, safe, EXACT_PROVABLE, unblocks the C1 conversation with real T4
   numbers.
10. **Production touch list on success:** C12 → `cuda_convert.cu` (decode
    ...
    C1 → `cuda_convert.cu` + `engine.cpp` decode seam
    (`decode_fp4_cache_block_to_scratch`) + new kernel file; C8 → host-side
    `engine.cpp` batching + `async_prefetcher`; none until gates pass.

## Verification performed here (no T4, no working nvcc)

- `pytest tests/test_t4_kernel_next.py` — 10 passed: traffic arithmetic,
  vec-vs-scalar index equivalence proof (3000 randomized + real-geometry
  spots + nibble-position proof), prototype/runner static properties,
  activation-fusion + launch-inventory source checks.

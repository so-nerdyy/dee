...
It makes no speedup claim. Flash's corrected result selects plain host LRU;
the VRAM priority repair is a separate explicit switch. The research verdict is
`NO_CODEC_WORTH_BUILDING`; `IdentityCodec` is the only codec implemented.
...
`PlainLruHostPlacementPolicy` is the selected production policy: all entries
are dynamic and the least-recently-used idle slot is evicted. Tests also use a
local first-eligible policy solely to create static-partition scenarios. The
interface remains generic for future research, but no frequency pinning,
activation collection, route prediction, or placement training is implemented.
Callbacks run under the host mutex, must not reenter the tier, and should avoid
blocking.

`DevicePlacementPolicy` remains a generic seam, but the selected VRAM repair is
`VramCacheManager::set_experimental_plain_lru(true)`: eviction uses
`last_used` only and removes the staging-order `priority*2^20` distortion.
It cannot evict pinned DMA/compute blocks or expand the byte budget. Device
access retains Engine's single-caller ownership contract.
